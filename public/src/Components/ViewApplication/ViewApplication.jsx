...
const viewApplication = async () => {
    let appDetails = await axios.get(`http://localhost:4000/admin/viewApplication/${id[0]}`);
  ...
  return (
    <>
     <Navbar  expand="lg" bg="secondary"  variant="white">
      <Container fluid>
        <Navbar.Brand  href="#"><Link to='/admin' style={{color:'Background',textDecorationLine:"none"}} className='panelHead' >Admin panel</Link></Navbar.Brand>
          ...
          <Card style={{ width: "100%" ,backgroundColor:'#f7f7f7'}}>
            <Card.Body>
              ...
              {app.bookingStat ? (<div>
                ...
                <div>
                <Button variant="primary" onClick={async()=>{
                const response = await axios.patch(`http://localhost:4000/admin/updateNewAppStatus/${id[0]}`)
                const stat = response.data
                setConfimration('Status updated')
              }
              }>Processing</Button>
              <Button variant="primary" style={{marginLeft:'1%',width:'100px'}} onClick={async()=>{
                const response = await axios.patch(`http://localhost:4000/admin/approveNewAppStatus/${id[0]}`)
                setConfimration('Status updated')
              }}>Approve</Button>
              <Button style={{marginLeft:'1%',width:'100px'}} onClick={async()=>{
                const response = await axios.patch(`http://localhost:4000/admin/rejectNewAppStatus/${id[0]}`)
                setConfimration('Status updated')
              }}>Reject</Button>
                </div>  
              )}
